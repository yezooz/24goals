...
class Save:
    ...
    def getTeamByEspn(self, name):
        ...
        team, created = Teams.objects.get_or_create(name_espn=name, defaults={'country_id': 1, 'country_code': 'pl'})
        return int(team.id)
    ...
    def save_match(self, match, source='e'):
        try:
            m = Matches.objects.get(espn_match_id=match["match_id"])
            return False  # reject if match exists
        except Matches.DoesNotExist:
            m = Matches()
            m.espn_match_id = match["match_id"]
            m.season_id = settings.CURRENT_SEASON_ID
        except TypeError:
            return False
        ...
        m.source = source
        m.league = self.getLeagueByEspn(match["league_name"])
        m.country = Countries.objects.get(pk=1)  # temp
...
if __name__ == '__main__':
    ...
    if len(args) > 0 and len(args) in (1, 2):
        if len(args) == 1:
            start_date = datetime.datetime.now().strftime('%Y%m%d')
        else:
            start_date = args[1]

        print "scanning whole league %s from %s" % (args[0], start_date)
...
Usage:
- fixtures league_name [start_date]"""
